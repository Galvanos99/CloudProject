...
def get_random_date(start_year=2018, end_year=2024):
    ...
    try:
        ...
        for i, item in enumerate(data):
            if "url" in item and item["media_type"] == "image":
                current_time = str(time.time()).encode()  # Pobierz aktualny czas w sekundach
                unique_id = str(hashlib.sha256(current_time).hexdigest()[:16])  # Skrót SHA-256 (pierwsze 16 znaków)
                metadata = {
                    "title": item["title"],
